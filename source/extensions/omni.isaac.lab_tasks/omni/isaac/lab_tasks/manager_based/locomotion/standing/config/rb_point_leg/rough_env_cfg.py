...
class RbPointLegRewardsCfg(RewardsCfg):
    ...
    undesired_contacts = RewTerm(
            ...
            weight=-1.0,
            params={"sensor_cfg": SceneEntityCfg("contact_forces", body_names=[".*HIP_link", ".*KNEE_link"]), "threshold": 1.0},
    )
    ...
    #     func=mdp.joint_deviation_l1,
    #     weight=-0.2,
    #     params={"asset_cfg": SceneEntityCfg("robot", joint_names=["L_foot_joint", "R_foot_joint"])},
    # )
...
@configclass
class RbPointLegRoughEnvCfg(LocomotionVelocityRoughEnvCfg):
    ...
    def __post_init__(self):
        super().__post_init__()
        # scene
        self.scene.robot = RB_POINT_LEG_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
...
@configclass
class RbPointLegRoughEnvCfg_PLAY(RbPointLegRoughEnvCfg):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()
...
@configclass
class RbPointLegRoughEnvCfg_PLAY_Env(RbPointLegRoughEnvCfg_PLAY):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()
        self.scene.terrain.terrain_type = "usd"
        self.scene.terrain.usd_path = f"{ISAAC_NUCLEUS_DIR}/Environments/Simple_Warehouse/wareouse.usd"